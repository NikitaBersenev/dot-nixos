...
help:
	@echo "Targets:"
	@echo "  make codex-tunnel       Initialize/check the OpenAI tunnel profile, then run it"
	@echo "  make codex-tunnel-init  Initialize/check the OpenAI tunnel profile only"
	@echo "  make codex-tunnel-run   Run an already initialized tunnel profile"
	@echo "  make codex-mcp          Run Codex MCP directly from this repository"
	@echo ""
	@echo "Config: copy .env.example to .env and set TUNNEL_ID and CONTROL_PLANE_API_KEY."

codex-mcp:
	@chmod +x tools/openai-tunnel/start-codex-mcp.sh
	@tools/openai-tunnel/start-codex-mcp.sh

codex-tunnel: codex-tunnel-init codex-tunnel-run
